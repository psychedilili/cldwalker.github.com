...
<notextile><pre class="console">
  ...
  irb>> commands 'lib --sort full_name --query_fields=full_name,description'
  +--------------+----------+-------+-------------------------------------------+-----------------------------------------------------------------------------+
  | full_name    | lib      | alias | usage                                     | description                                                                 |
  +--------------+----------+-------+-------------------------------------------+-----------------------------------------------------------------------------+
  | install      | web_core |       | [url][--force] [--name=NAME]              | Installs a library by url. Library should then be loaded with load_library. |
  ...
  # In your shell
  bash> boson commands lib --sort full_name --query_fields=full_name,description
  # same as above ...
...
<notextile><pre class='console'>
  ...
  | --query_fields | -q    | array   |                | name,lib,alias,description,options,args,usage,full_name,render_options |
  +----------------+-------+---------+----------------+------------------------------------------------------------------------+
...
To try some third-party Boson libraries, "here":http://github.com/cldwalker/irbfiles/blob/master/boson/commands/site/github.rb "are":http://github.com/cldwalker/irbfiles/blob/master/boson/commands/ruby_ref.rb "some":http://github.com/cldwalker/irbfiles/blob/master/boson/commands/ri.rb of mine I recommend.
