...
keywords: irb ruby, ruby irb, irb configuration, irb conf, irb tutorial, irb documentation, how to configure irb, irb config options
description: In <a href="/2009/05/11/demystifying-irb-commands.html">my last post</a> we discussed irb's commands. This post goes further into irb's waters, exploring <b>all</b> of it's configuration options.
tags:
  - lib:name=irb
  - post:type=tutorial
  - post:lang=ruby
...
* Irb saves it's configuration in a hash accessible through @IRB.conf@. By convention, the config keys are symbols in all caps. Simply set an option by setting a key's value: @IRB.conf[:SAVE_HISTORY] = 1000@
...
If @:HISTORY_FILE@ isn't set, irb uses the same directory and file format of the detected irbrc for the irb history file. For example, ~/.irbrc leads to a ~/.irb_history whereas a ./irbrc leads to ./irb_history. For further discussion on this read ":RC_NAME_GENERATOR":#rc_name_generator.
...
A common irb complaint is that it's output is too verbose. Disabling this option makes irb print __no output__. Here's a handy snippet that uses this option to toggle irb output:
...
The different prompt types that start with :PROMPT* can take special string flags starting with %. For more details on this I refer to the section "Configuring the Prompt in the pickaxe":http://www.rubycentral.com/book/irb.html.
