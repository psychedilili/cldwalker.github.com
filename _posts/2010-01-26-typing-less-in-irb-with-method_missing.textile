...
description: Being the console-efficient prick I am, let's look at a <code>method_missing</code> that let's us type less where underscored methods are involved. This means that instead of typing <code>some_really_long_ass_method</code> we can type <code>s_r_l_a_s</code>.
tags:
...
{{page.description}} So why do this if we have autocompletion? For starters, try typing the Object method @instance_variable_get@ in five keystrokes (tabs included). Or for more inefficiency, try typing @ActiveRecord@ class methods that start with @before_*@ and @validate_*@.

h2. Explanation

To write this @method_missing@, we're going to need a method that finds methods given a method query we type:
...
<notextile><pre class='console'>
  ...
  # If our method query is too broad we get multiple results
...
When using the above snippet, you may find it annoying that sometimes a method query results in a multiple choices that vary only by ending i.e. @updated_at? updated_at updated_at=@. If only there was a way to autocomplete like this ... Actually, "bond":http://github.com/cldwalker/bond "accomplished this a while ago":http://tagaholic.me/2009/07/16/bond-from-irb-with-completion-love.html#autocompletion_aliasing.

Now a general question to you all: Would anyone find it useful to apply this same instant aliasing technique to their rake/thor commandline tasks? For example, typing r:u:s instead of rails:update:scripts? I have found it quite useful in instantly aliasing "boson's":http://github.com/cldwalkwer/boson commands, arguments and options.
